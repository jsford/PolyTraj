...
protected:
   ...
   * @param c [description]
   */
  void getGeneralizedCauchyPoint(const TProblem &problem, const TVector &x, const TVector &g, TVector &x_cauchy,
                                 VariableTVector &c) {
  ...
  void minimize(TProblem &problem, TVector &x0) {
    ...
    auto noConvergence =
      [&](TVector &x, TVector &g) -> bool {
        return (((x - g).cwiseMax(problem.lowerBound()).cwiseMin(problem.upperBound()) -
                 x).template lpNorm<Eigen::Infinity>() >= 1e-4);
      };
