  ...
  */
  void minimize(TProblem &objFunc, TVector &x0, const TVector &var0) {
    ...
    do {
      ...
      Scalar hsig = (ps.norm() / sqrt(pow(1 - (1. - cs), (2 * (this->m_current.iterations + 1)))) < hsig_thr) ? 1.0
                                                                                                              : 0.0;
      ...
      if (Super::m_debug >= DebugLevel::Low) {
        std::cout << "Iteration: " << this->m_current.iterations << " best cost " << costs[indices[0]] << " sigma "
                  << sigma
