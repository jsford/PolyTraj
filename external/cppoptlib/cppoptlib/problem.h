...
#pragma GCC diagnostic ignored "-Wunused-parameter"
  ...
  virtual bool detailed_callback(const Criteria <Scalar> &state, SimplexOp op, int index, const MatrixType &x,
                                 std::vector<Scalar> f) {
    return true;
  }
  ...
  virtual bool checkHessian(const TVector &x, int accuracy = 3) {
    ...
    for (TIndex d = 0; d < D; ++d) {
      for (TIndex e = 0; e < D; ++e) {
        Scalar scale = std::max(static_cast<Scalar>(std::max(fabs(actual_hessian(d, e)), fabs(expected_hessian(d, e)))),
                                Scalar(1.));
